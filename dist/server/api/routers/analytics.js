...
export const analyticsRouter = router({
    ...
    history: adminProcedure
        ...
        let conditions = [];
        if (input.channelId)
            conditions.push(eq(posts.channelId, input.channelId));
        if (input.startDate)
            conditions.push(gte(posts.createdAt, input.startDate));
        if (input.endDate)
            conditions.push(lte(posts.createdAt, input.endDate));
